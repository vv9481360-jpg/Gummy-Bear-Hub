--=============================================================
--  GUMMY BEAR HUB v9.0 — All-in-One
--  Панель + BloxStrike чит внутри
--=============================================================
...
--=============================================================
--  16. ВКЛАДКА "BLOXSTRIKE" (появляется только в BloxStrike)
--=============================================================
...
if game.PlaceId == BLOXSTRIKE_ID then
    ...
    makeDivider(bsPage, "🎯 Точность стрельбы")
    ...
    makeToggle(bsPage, "No Recoil / Spread / Camera", false, function(state)
        if state then
            ...
            print("[BS] Перезайди в игру, чтобы выключить")
        end
    end)
    
    makeDivider(bsPage, "👁️ Визуал")
    
    -- ESP
    local espFolder = new("Folder", { Parent = ScreenGui, Name = "BS_ESP" })
    local espActive = false
    
    makeToggle(bsPage, "Player ESP", false, function(state)
        espActive = state
        ...
        print("[BS] ESP включён")
    end)
    
    makeDivider(bsPage, "🌍 Мир")
...
print("[Gummy Bear Hub] Загружено! v" .. CONFIG.Version .. " | Place ID: " .. tostring(game.PlaceId))
if game.PlaceId == BLOXSTRIKE_ID then
